...
def train(model,metric,dataloader,criterion,optimizer,epoch,device):
    ...
    for batch_id, data in enumerate(tqdm(dataloader)):
        ...
        optimizer.zero_grad()
        (output1, output2),(emb1,emb2) = model(inputs0,inputs1)
    ...
    #         p1.append(emb2.cpu().detach().numpy())
        ...
        print('!!!', np.array(metrics.cpu().detach().numpy()), targets.cpu().detach().numpy(), '!!!\n')
        t_acc = np.sum(np.round(metrics.cpu().detach().numpy()) == targets.cpu().detach().numpy())
    ...
    return train_loss, train_acc * 100,(emb1,emb2)# p0, p1
def fun(l):
    if l>0.6:
        return 1.
    else:
...
def test(model, metric, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for batch_id, data in enumerate(tqdm(dataloader)):
            ...
            # optimizer.zero_grad()
            (output1, output2),emb = model(inputs0,inputs1)
            metrics = metric(output1,output2).squeeze()
            ...
            print('!!!',metrics.cpu().detach().numpy(),targets.cpu().detach().numpy(),'!!!\n')
            # t_acc = np.sum(np.array(list(map(fun, metrics.cpu().detach().numpy()))) == targets.cpu().detach().numpy())
            print('!!!', np.array(metrics.cpu().detach().numpy()), targets.cpu().detach().numpy(), '!!!\n')
            t_acc = np.sum(np.round(metrics.cpu().detach().numpy()) == targets.cpu().detach().numpy())
            test_acc += t_acc
